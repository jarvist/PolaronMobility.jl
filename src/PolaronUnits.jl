...
function addunits!(polaron::FrohlichPolaron; unit="pu")
    ...
    polaron.Fs = pustrip.(polaron.Fs) .* punit(1Unitful.meV)
    polaron.Fl = pustrip.(polaron.Fl) .* punit(1Unitful.meV)
    polaron.Ms = pustrip.(polaron.Ms) .* punit(1Unitful.me)
    polaron.Ml = pustrip.(polaron.Ml) .* punit(1Unitful.me)
    polaron.Rs = pustrip.(polaron.Rs) .* punit(1Unitful.Å)
    polaron.Rl = pustrip.(polaron.Rl) .* punit(1Unitful.Å)
    ...
    polaron.κ0 = pustrip.(polaron.κ0) .* punit(1u"μN/m")
    polaron.M0 = pustrip.(polaron.M0) .* punit(1Unitful.me)
    polaron.M0a = pustrip.(polaron.M0a) .* punit(1Unitful.me)
    polaron.M0r = pustrip.(polaron.M0r) .* punit(1Unitful.me)
    polaron.R0 = pustrip.(polaron.R0) .* punit(1Unitful.Å)
    polaron.T = pustrip.(polaron.T) .* punit(1Unitful.K)
    polaron.β = pustrip.(polaron.β) .* punit(1 / 1Unitful.K)
    ...
    polaron.κ = pustrip.(polaron.κ) .* punit(1u"μN/m")
    polaron.M = pustrip.(polaron.M) .* punit(1Unitful.me)
    polaron.Ma = pustrip.(polaron.Ma) .* punit(1Unitful.me)
    polaron.Mr = pustrip.(polaron.Mr) .* punit(1Unitful.me)
    polaron.R = pustrip.(polaron.R) .* punit(1Unitful.Å)
    polaron.μ = pustrip.(polaron.μ) .* punit(1u"cm^2/V/s")
    polaron.μFHIP = pustrip.(polaron.μFHIP) .* punit(1u"cm^2/V/s")
    polaron.μD = pustrip.(polaron.μD) .* punit(1u"cm^2/V/s")
    polaron.μK = pustrip.(polaron.μK) .* punit(1u"cm^2/V/s")
    polaron.μH = pustrip.(polaron.μH) .* punit(1u"cm^2/V/s")
    polaron.μH0 = pustrip.(polaron.μH0) .* punit(1u"cm^2/V/s")
    polaron.τ = pustrip.(polaron.τ) .* punit(1u"ns")
    polaron.Ω = pustrip.(polaron.Ω) .* punit(1Unitful.THz)
    polaron.χ = pustrip.(polaron.χ) .* punit(1Unitful.THz2π)
    polaron.z = pustrip.(polaron.z) .* punit(u"Ω")
    polaron.σ = pustrip.(polaron.σ) .* punit(u"S")
    if unit == "au"
        auconvert!(polaron)
    elseif unit == "su"
        suconvert!(polaron)
...
function addunits!(polaron::HolsteinPolaron; unit="pu")
    polaron.ω = phustrip.(polaron.ω) .* phunit(1Unitful.THz2π)
    polaron.ωeff = phustrip.(polaron.ωeff) .* phunit(1Unitful.THz2π)
    polaron.F0 = phustrip.(polaron.F0) .* phunit(1Unitful.meV)
    polaron.A0 = phustrip.(polaron.A0) .* phunit(1Unitful.meV)
    polaron.B0 = phustrip.(polaron.B0) .* phunit(1Unitful.meV)
    polaron.C0 = phustrip.(polaron.C0) .* phunit(1Unitful.meV)
    polaron.v0 = pustrip.(polaron.v0) .* punit(1Unitful.meV / 1Unitful.ħ)
    polaron.w0 = pustrip.(polaron.w0) .* punit(1Unitful.meV / 1Unitful.ħ)
    polaron.κ0 = phustrip.(polaron.κ0) .* phunit(1u"μN/m")
    polaron.M0 = phustrip.(polaron.M0) .* phunit(1Unitful.me)
    polaron.M0a = phustrip.(polaron.M0a) .* phunit(1Unitful.me)
    polaron.M0r = phustrip.(polaron.M0r) .* phunit(1Unitful.me)
    polaron.R0 = phustrip.(polaron.R0) .* phunit(1Unitful.Å)
    polaron.T = phustrip.(polaron.T) .* phunit(1Unitful.K)
    polaron.β = phustrip.(polaron.β) .* phunit(1 / 1Unitful.K)
    polaron.F = phustrip.(polaron.F) .* phunit(1Unitful.meV)
    polaron.A = phustrip.(polaron.A) .* phunit(1Unitful.meV)
    polaron.B = phustrip.(polaron.B) .* phunit(1Unitful.meV)
    polaron.C = phustrip.(polaron.C) .* phunit(1Unitful.meV)
    polaron.v = pustrip.(polaron.v) .* punit(1Unitful.meV / 1Unitful.ħ)
    polaron.w = pustrip.(polaron.w) .* punit(1Unitful.meV / 1Unitful.ħ)
    polaron.κ = phustrip.(polaron.κ) .* phunit(1u"μN/m")
    polaron.M = phustrip.(polaron.M) .* phunit(1Unitful.me)
    polaron.Ma = phustrip.(polaron.Ma) .* phunit(1Unitful.me)
    polaron.Mr = phustrip.(polaron.Mr) .* phunit(1Unitful.me)
    polaron.R = phustrip.(polaron.R) .* phunit(1Unitful.Å)
    polaron.μ = phustrip.(polaron.μ) .* phunit(1u"cm^2/V/s")
    polaron.Ω = phustrip.(polaron.Ω) .* phunit(1Unitful.THz)
    polaron.χ = phustrip.(polaron.χ) .* phunit(1Unitful.THz2π)
    polaron.z = phustrip.(polaron.z) .* phunit(u"Ω")
    polaron.σ = phustrip.(polaron.σ) .* phunit(u"S")
    if unit == "au"
        auconvert!(polaron)
    elseif unit == "su"
        suconvert!(polaron)
...
function suconvert!(polaron::FrohlichPolaron)
    ...
    polaron.χ = polaron.χ .|> Unitful.THz2π
