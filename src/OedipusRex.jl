...
optical_absorption(Ω::Float64, α::Float64, v::Float64, w::Float64)
    ...
    (equation (11a) in [1]) for a given frequency Ω. v and w are the
...
optical_absorption(Ω::Float64, β::Float64, α::Float64, v::Float64, w::Float64)
    ...
    Calculate the absorption coefficient Γ(Ω) for the polaron at finite temperatures
    (equation (11b) in [1]) for a given frequency Ω. β is the thermodynamic
    beta. v and w are the variational Polaron parameters that minimise the free energy, for
    the supplied α Frohlich coupling.
