...
#     integral, error = ω^2 .* quadgk(x -> integrand(x), 0.0, Inf64, rtol = rtol)
...
#     # Integrate using adapative quadrature algorithm with relative error tolerance rtol.
#     integral, error = ω .* quadgk(x -> integrand(x), 0.0, Inf64, rtol = rtol)
...
function polaron_memory_function_thermal(Ω, β, α, v, w; ω = 1.0, rtol = 1e-3)
    ...
    @tullio integral[j] := quadgk(t -> integrand(t, β[j], Ω / ω[j]), 0.0, Inf64, rtol = rtol)

    @tullio (+) memory := 2 * α[j] * ω[j]^2 * integral[j][1] / (3 * √π * Ω * 2π)
...
function polaron_memory_function_athermal(Ω, α, v, w; ω = 1.0, rtol = 1e-3)
    ...
    @tullio integral[j] := Ω < minimum(ω) / 2π ? (0.0 + im * 0.0, 0.0) : quadgk(t -> integrand(t, Ω / ω[j]), 0.0, 1 / rtol, rtol = rtol)
...
function polaron_memory_function_dc(β, α, v, w; ω = 1.0, rtol = 1e-3)
    ...
    @tullio integral[j] := quadgk(t -> integrand(t, β[j]), 0.0, 1 / rtol, rtol = rtol)

    @tullio (+) memory := 2 * α[j] * ω[j] * integral[j][1] / (3 * √π)
