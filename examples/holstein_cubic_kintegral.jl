...
# ╔═╡ a1edbce2-7f94-11ed-192e-8fb596f506f2
begin
    using Optim
    using QuadGK
...
# ╔═╡ f0e39286-1b85-46fd-afa2-9f01ee37e0c6
unzip(a) = map(x->getfield.(a, x), fieldnames(eltype(a)))

# ╔═╡ 54b6f0bf-d95a-4366-895b-466d61b06e09
function D(τ, v, w)
...
# ╠═╡ disabled = true
#=╠═╡
vh_s, wh_s, Eh_s = unzip(frohlich_vw_k.(holstein_matrix, α_range, cutoff = 2π * (3/4π)^(1/3)))
  ╠═╡ =#

# ╔═╡ 4fda0925-c3de-4e3a-a788-c01ee9fb2f70
# ╠═╡ disabled = true
#=╠═╡
...
#=╠═╡
begin
	plot(α_range, vf, label = "vf", linewidth = 2.0, xticks = 0:1:12)
	plot!(α_range, wf, label = "wf", linewidth = 2.0)
	plot!(α_range, vh, label = "vh", linewidth = 2.0, linestyle = :dash)
	plot!(α_range, wh, label = "wh", linewidth = 2.0, linestyle = :dash)
	plot!(α_range, vh_c, label = "vh_c", linewidth = 2.0, linestyle = :dot)
	plot!(α_range, wh_c, label = "wh_c", linewidth = 2.0, linestyle = :dot)
end
  ╠═╡ =#

# ╔═╡ 0e4e18f5-9687-43a0-8884-0c2f0b1a2e4a
#=╠═╡
begin
	plot(α_range, Ef, label = "Ef", linewidth = 2.0, xticks = 0:1:12)
	plot!(α_range, Eh, label = "Eh", linewidth = 2.0, linestyle = :dash)
	plot!(α_range, Eh_c, label = "Eh_c", linewidth = 2.0, linestyle = :dot)
end
  ╠═╡ =#
